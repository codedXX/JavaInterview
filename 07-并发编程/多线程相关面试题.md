...
<img src="images/image-20221026105350827.png" alt="image-20221026105350827" />
...
<img src="images/image-20221026105442158.png" alt="image-20221026105442158" />
...
<img src="images/image-20230503203246348.png" alt="image-20230503203246348" />



<img src="images/image-20221026105607248.png" alt="image-20221026105607248" />

多核CPU

每个核（core）都可以调度运行线程，这时候线程可以是并行的。

<img src="images/image-20230503203330700.png" alt="image-20230503203330700" />
...
<img src="images/image-20230503203629212.png" alt="image-20230503203629212" />
...
> ⭐️⭐️**这个线程的状态，比如说现在是有执行资格和执法权了，但进入到了waiting，那waiting过后又要进入有执行资格没有执法权的状态继续等待cpu执行权吗？**
>
> 
>
> ```
...
> 等待 WAITING
>   ↓ notify()
> 就绪 RUNNABLE（有执行资格，没有执行权）
>   ↓ 抢到 CPU
> 运行 RUNNING
> ```
>
> 所以你的理解是对的：`WAITING` 结束后，先回到“有执行资格、没有执行权”的就绪状态，再等待 CPU。
>
> 
>
>
> ⭐️⭐️**那要按照你这样说的话，比如说我sleep(500)，那我500ms后不能马上执行，就相当于sleep的时间其实是大于500的，对吗？**
>
> 对。`sleep(500)` 表示线程至少休眠约 500ms，而不是保证 500ms 后立刻执行。
>
> 状态变化是：
>
> ```
...
> ```
>
> 因此实际耗时通常会大于 500ms，因为到期后还要等待操作系统调度 CPU。除非线程被 `interrupt()`，否则不会提前结束休眠。
...
为了确保三个线程的顺序你应该先启动最后一个(`T3调用T2，T2调用T1`)，这样`T1就会先完`成而`T3最后完成`
...
>
> * `共同点`
>   * wait() ，wait(long) 和 sleep(long) 的效果都是让`当前线程暂时放弃 CPU 的使用权，进入阻塞状态`
>
> * `不同点`
>
>   * `方法归属不同`
...
> ⭐️**注意：**
>
> * `wait方法一定要和synchronized 配置使用`，不然会报错的
...
<img src="images/image-20230504165342501.png" alt="image-20230504165342501" />
...
<img src="images/image-20230504165833809.png" alt="image-20230504165833809" />
...
### 2.2 synchronized关键字的底层原理-进阶(我跳过了)
...
<img src="images/image-20230504172253826.png" alt="image-20230504172253826" />

我们需要重点分析MarkWord对象头

#### 2.2.2 MarkWord

<img src="images/image-20230504172541922.png" alt="image-20230504172541922" />
...
<img src="images/image-20230504172957271.png" alt="image-20230504172957271" />
...
<img src="images/image-20230504173520412.png" alt="image-20230504173520412" />

2.通过CAS指令将Lock Record的地址存储在对象头的mark word中（数据进行交换），如果对象处于无锁状态则修改成功，代表该线程获得了轻量级锁。

<img src="images/image-20230504173611219.png" alt="image-20230504173611219" />



3.如果是当前线程已经持有该锁了，代表这是一次锁重入。设置Lock Record第一部分为null，起到了一个重入计数器的作用。

<img src="images/image-20230504173922343.png" alt="image-20230504173922343" />
...
<img src="images/image-20230504173955680.png" alt="image-20230504173955680" />

3.如果Lock Record的 Mark Word不为null，则利用CAS指令将对象头的mark word恢复成为无锁状态。如果失败则膨胀为重量级锁。

<img src="images/image-20230504174045458.png" alt="image-20230504174045458" />
...
<img src="images/image-20230504174525256.png" alt="image-20230504174525256" />

2.通过CAS指令将Lock Record的**线程id**存储在对象头的mark word中，同时也设置偏向锁的标识为101，如果对象处于无锁状态则修改成功，代表该线程获得了偏向锁。

<img src="images/image-20230504174505031.png" alt="image-20230504174505031" />

3.如果是当前线程已经持有该锁了，代表这是一次锁重入。设置Lock Record第一部分为null，起到了一个重入计数器的作用。与轻量级锁不同的时，这里不会再次进行cas操作，只是判断对象头中的线程id是否是自己，因为缺少了cas操作，性能相对轻量级锁更好一些

<img src="images/image-20230504174736226.png" alt="image-20230504174736226" />
...
> 
>
> 把 Java 多线程想成几个人一起改一份共享表格。
>
> - **主内存**：那份真正的公共表格。
> - **线程 A / B**：两个正在干活的人。
> - **工作内存**：每个人桌上拿的一份表格副本。
>
> 问题是：A 改了自己桌上的副本，B 不会立刻知道。A 需要把改动交回公共表格；B 也需要重新从公共表格拿最新内容。
>
> 所以可能出现：
>
> 1. A 把 `count` 从 `0` 改成 `1`，但只是改了自己的副本。
> 2. B 仍然看着旧副本，以为 `count` 还是 `0`。
> 3. 两人都基于旧值修改，最终有一次修改会被覆盖。
>
> JMM 就是 Java 定的一套“大家什么时候把副本同步到公共表格、什么时候必须读取最新公共表格”的规则。
>
> `volatile` 可以理解为：变量一有变化就立刻公开，别人读取时必须看最新值。
>
> `synchronized` 可以理解为：给共享表格上锁，同一时间只能一个人修改，改完再让下一个人进来。



Java内存模型(Java Memory Model)描述了Java程序中各种变量(线程共享变量)的访问规则，以及在JVM中将变量存储到内存和从内存中读取变量这样的底层细节。

<img src="images/image-20230504181638237.png" alt="image-20230504181638237" />

**特点：**

1. 所有的`共享变量`都存储于主内存(计算机的RAM)这里所说的`变量`指的是实例变量和类变量。`不包含局部变量`，因为局部变量是线程私有的，因此不存在竞争问题。
...
- JMM(Java Memory Model)Java内存模型，定义了`共享内存中多线程程序` `读写操作`的行为规范，通过这些规则来规范对内存的读写操作从而保证指令的正确性
...
**CAS的全称是：** Compare And Swap(比较再交换)，它体现的一种乐观锁的思想，在`无锁情况下`保证`线程操作共享数据的原子性`。
...
<img src="images/image-20230504181947319.png" alt="image-20230504181947319" />

>一个当前主内存值V、旧的预期值A、即将更新的值B，当且仅当旧的预期值A和内存值V相同时，将内存值修改为B并返回true，否则什么都不做，并返回false。如果CAS操作失败，通过自旋的方式等待并再次尝试，直到成功

- **线程1操作：**V：int a = 100，A：int a = 100，B：修改后的值：int a = 101 (a++)
  - 线程1拿A的值与主内存V的值进行比较，判断是否相等
  - 如果相等，则把B的值101更新到主内存中

<img src="images/image-20230504182129820.png" alt="image-20230504182129820" />

- **线程2操作：**V：int a = 100，A：int a = 100，B：修改后的值：int a = 99(a--)
  - 线程2拿A的值与主内存V的值进行比较，判断是否相等(目前不相等，因为线程1已更新V的值99)
  - 不相等，则线程2更新失败
  - ⭐️`当前线程2想要同步数据成功的话，就需要从共享内存再读一份数据，读完之后a就变成101，然后再做a--操作，再把数据同步过去，这时候就能成功了`。这就是`自旋锁`

<img src="images/image-20230504181827330.png" alt="image-20230504181827330" />
...
<img src="images/image-20230504182447552.png" alt="image-20230504182447552" />
...
<img src="images/image-20230504182737931.png" alt="image-20230504182737931" />

都是native修饰的方法，由系统提供的接口执行，并非java代码实现，一般的思路也都是自旋锁实现

<img src="images/image-20230504182838426.png" alt="image-20230504182838426" />

在java中比较常见使用有很多，比如ReentrantLock和Atomic开头的线程安全类，都调用了Unsafe中的方法

- ReentrantLock中的一段CAS代码

<img src="images/image-20230504182958703.png" alt="image-20230504182958703" />
...
- synchronized 是基于悲观锁的思想：`最悲观`的估计，得防着其它线程来修改共享变量，我上了锁你们都别想改，我改完了解开锁，你们才有机会。
...
public class ForeverLoop {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            ...
            System.out.println(Thread.currentThread().getName() + ": " + stop); // true
        }, "t2").start();
...
> 注意：
>
> * ⭐️线程2打印的值是true，说明`线程间是可见的`

当执行上述代码的时候，发现`foo()方法中的循环是结束不了的，也就说读取不到共享变量的值结束循环`。

主要是因为在**JVM虚拟机中有一个JIT（即时编辑器）给代码做了优化。**
...
<img src="images/image-20230505082441116.png" alt="image-20230505082441116" />
...
<img src="images/image-20230505082835588.png" alt="image-20230505082835588" />

屏障添加的示意图

<img src="images/image-20230505082923729.png" alt="image-20230505082923729" />
...
<img src="images/image-20230505083124159.png" alt="image-20230505083124159" />

屏障添加的示意图

<img src="images/image-20230505083217904.png" alt="image-20230505083217904" />
...
> 这张图想表达的是：在多线程环境中，为了提高执行效率，Java 或 CPU 有时会调整代码的实际执行顺序。这种调整叫作“指令重排序”。
>
> 例如，线程 A 的代码是：
>
> ```
...
> ```
>
> 按代码顺序理解，线程 A 应该先给 `x` 赋值，再给 `y` 赋值。因此，如果线程 B 已经读到 `y = 1`，我们通常会认为它接着也应读到 `x = 1`。
>
> 但在没有加同步控制时，Java 或 CPU 为了优化性能，可能让线程 A 实际上先执行：
>
> ```
> y = 1;
> x = 1;
...
> ```
>
> 最终结果就是 `r1 = 1，r2 = 0`。这就是图片中说的“指令重排序影响结果”。
>
> `volatile` 的作用之一，就是限制这种重排序。比如将 `y` 声明为 `volatile` 后，线程 B 一旦读到 `y = 1`，就也能看到线程 A 在此之前对 `x` 做的修改。
...
全称是 `A`bstract`Q`ueued`S`ynchronizer，是阻塞式锁和相关的同步器工具的框架，它是构建锁或者其他同步组件的`基础框架`
...
<img src="images/image-20230505083840046.png" alt="image-20230505083840046" />
...
<img src="images/image-20230505084451193.png" alt="image-20230505084451193" />

在去修改state状态的时候，使用的cas自旋锁来`保证原子性`，确保只能有一个线程修改成功，修改失败的线程将会进入FIFO队列中等待

> ⭐️`原子性`指的是：一个操作在并发环境下要么完整成功，要么完全不发生，其他线程不能看到它“做到一半”的状态。



**AQS是公平锁吗，还是非公平锁？**

- `新`的线程与`队列`中的线程共同来抢资源，是`非公平锁`

- `新`的线程到队列中等待，只让队列中的head线程获取锁，是`公平锁`
...
- 是多线程中的队列同步器。是一种锁机制，它是做为一个`基础框架`使用的，像 ReentrantLock、Semaphore 都是基于 AQS 实现的。
...
<img src="images/1.png" alt="image-20230505084451193" />
...
<img src="images/image-20230505091736569.png" alt="image-20230505091736569" />
...
<img src="images/image-20230505091827720.png" alt="image-20230505091827720" />
...
<img src="images/image-20230505092151244.png" alt="image-20230505092151244" />

而Sync的父类是AQS，所以可以得出ReentrantLock底层主要实现就是基于AQS来实现的

<img src="images/image-20230505091833629.png" alt="image-20230505091833629" />

**工作流程**

<img src="images/image-20230505092340431.png" alt="image-20230505092340431" />
...
~~~java
// Lock 接口，定义了加锁、解锁等基本操作。
import java.util.concurrent.locks.Lock;

// ReentrantLock 是 Lock 接口最常用的实现类之一。
import java.util.concurrent.locks.ReentrantLock;

/**
 * 用一个共享计数器，演示 synchronized 和 Lock 的基本区别。
 *
 * 运行这个类后，两个线程会同时累加两个计数器：
 * 1. SyncCounter 使用 synchronized；
 * 2. LockCounter 使用 ReentrantLock。
 *
 * 如果加锁正确，两个计数器最终都应该是 20000。
 */
public class SynchronizedVsLockDemo {

    /**
     * 使用 synchronized 实现线程安全的计数器。
     */
    static class SyncCounter {
        // 两个线程会同时修改这个变量，因此它是共享数据。
        private int count = 0;

        /**
         * 计数器加一。
         *
         * synchronized 会在进入方法时自动获取当前对象的锁，
         * 方法执行结束后自动释放锁，即使方法中出现异常也会释放。
         */
...
//        public void increment() {
    ...
    static class LockCounter {
        ...
        public void increment() {
            ...
            try {
                // 获取锁后，当前线程可以安全地修改共享变量。
                count++;
            } finally {
                // Lock 不会自动释放，必须手动 unlock。
                // 放在 finally 中可以确保出现异常时也能释放锁，避免死锁。
                lock.unlock();
            }
        }
     ...
     * @throws InterruptedException 等待线程结束时可能被中断
     */
    public static void main(String[] args) throws InterruptedException {
        // 创建 synchronized 版本的计数器。
        SyncCounter syncCounter = new SyncCounter();

        // 创建 Lock 版本的计数器。
        LockCounter lockCounter = new LockCounter();

        // 创建第一个线程，让它重复执行 10000 次加一操作。
        // Lambda 表达式中的代码会在线程启动后执行。
        Thread thread1 = new Thread(() -> incrementManyTimes(syncCounter, lockCounter));

        // 创建第二个线程，它和第一个线程操作同一批计数器。
        // 两个线程并发执行，正好可以观察锁的作用。
        Thread thread2 = new Thread(() -> incrementManyTimes(syncCounter, lockCounter));

        // 启动第一个线程。start() 会真正创建并运行新的线程。
        thread1.start();

        // 启动第二个线程。此时两个线程可能交替执行。
        thread2.start();

        // 等待第一个线程执行完毕。
        // 如果不 join，主线程可能提前打印结果。
        thread1.join();

        // 等待第二个线程执行完毕。
        thread2.join();

        // 两个线程各加 10000 次，因此正确结果应该是 20000。
        System.out.println("synchronized: " + syncCounter.getCount());
        System.out.println("Lock:          " + lockCounter.getCount());
    }

    /**
     * 让一个线程同时操作两个计数器。
     *
     * @param syncCounter synchronized 版本的计数器
     * @param lockCounter ReentrantLock 版本的计数器
     */
    private static void incrementManyTimes(SyncCounter syncCounter, LockCounter lockCounter) {
        // 每个线程执行 10000 次加一操作。
        for (int i = 0; i < 10_000; i++) {
            // 调用 synchronized 版本的加一方法。
...
> ```
>
> 然后多运行几次，结果可能小于 `20000`，因为两个线程同时执行 `count++` 会产生数据丢失。
>
> 恢复原来的 `synchronized` 和 `Lock` 代码后，结果就会稳定为：
>
> ```java
> 20000
> ```
...
> 
>
> ⭐**为什么public synchronized void increment()改成public void increment() 后就不会稳定到20000？**
>
> 因为 `count++` 不是一个不可分割的操作，它实际上包含三步：
>
> ```java
> 读取 count
> count + 1
...
> ```
>
> 两个线程实际上执行了两次加一，但最终只增加了 1，这就是“数据丢失”。
>
> 加上：
>
> ```java
...
> ```
>
> 后，同一个 `SyncCounter` 对象同一时刻只允许一个线程进入 `increment()`，因此操作会变成：
>
> ```java
> 线程 A：读取 5 → 加一 → 写回 6
> 线程 B：读取 6 → 加一 → 写回 7
...
> ```
>
> 去掉 `synchronized` 后，结果不一定每次都小于 `20000`，有时也可能碰巧是 `20000`，因为线程执行顺序是不确定的；但它不再有并发安全保证。
>
> 
>
> ⭐**为什么要thread1.join();和thread2.join()？**
>
> `join()` 的作用是：让主线程等待指定线程执行结束。
>
> ```java
> thread1.join();
> thread2.join();
...
> 因为两个工作线程还没有加完一万次。
>
> 注意：这两行不会让两个线程变成串行执行。因为前面已经启动了两个线程：
>
> ```
> thread1.start();
> thread2.start();
...
> 主线程：启动 thread1
> 主线程：启动 thread2
> 主线程：等待 thread1 和 thread2
> 主线程：打印最终结果
> ```
>
> 因此，`join()` 保证打印时两个线程已经完成，最终结果才应该是：
>
> ```
> synchronized: 20000
> Lock:          20000
...
<img src="images/image-20220902171032898.png" alt="image-20220902171032898" />
...
当程序出现了死锁现象，我们可以使用`jdk自带的工具`：`jps`和` jstack`

**步骤如下：**

* **第一：查看运行的线程**

<img src="images/image-20220902171426738.png" alt="image-20220902171426738" />

* **第二：使用jstack查看线程运行的情况，下图是截图的关键信息**
  * 运行命令：`jstack -l 46032`

<img src="images/image-20220902172229567.png" alt="image-20220902172229567" />
...
<img src="images/image-20230505092654811.png" alt="image-20230505092654811" />
...
<img src="images/image-20230505093055382.png" alt="image-20230505093055382" />
...
<img src="images/image-20230505093507265.png" alt="image-20230505093507265" />
...
<img src="images/image-20230505205200628.png" alt="image-20230505205200628" />

以上代码会出现超卖或者是一张票卖给多个人，执行并不是原子性的
...
<img src="images/image-20230505210853493.png" alt="image-20230505210853493" />
...
<img src="images/image-20230505211002252.png" alt="image-20230505211002252" />
...
<img src="images/image-20230505211209336.png" alt="image-20230505211209336" />
...
<img src="images/image-20230505220514872.png" alt="image-20230505220514872" />
...
<img src="images/image-20230505220701835.png" alt="image-20230505220701835" />
...
**比较常见的有4个**，用的最多是`ArrayBlockingQueue和LinkedBlockingQueue`
...
<img src="images/image-20230505221424359.png" alt="image-20230505221424359" />
...
<img src="images/image-20230505221837189.png" alt="image-20230505221837189" />
...
在java.util.concurrent.Executors类中提供了大量**创建连接池的静态方法，常见就有四种**

1. **创建使用固定线程数的线程池**

   <img src="images/image-20230505221959259.png" alt="image-20230505221959259" />
...
2. **单线程化的线程池，它只会用唯一的工作线程来执行任 务，保证所有任务按照指定顺序(FIFO)执行**
   ...
   <img src="images/image-20230505222050294.png" alt="image-20230505222050294" />
...
3. **可缓存线程池**
   ...
   <img src="images/image-20230505222126391.png" alt="image-20230505222126391" />
...
4. **提供了“延迟”和“周期执行”功能的ThreadPoolExecutor。**
   ...
   <img src="images/image-20230505222203615.png" alt="image-20230505222203615" />
...
<img src="images/image-20220821003816845.png" alt="image-20220821003816845" />
...
**CountDownLatch（闭锁/倒计时锁）用来进行线程同步协作，`等待所有线程`完成倒计时（一个或者多个线程，等待其他多个线程完成某件事情之后才能执行）**
...
<img src="images/image-20230505223014946.png" alt="image-20230505223014946" />
...
/**
 * CountDownLatch（倒计时门闩）示例。
 *
 * 主线程创建三个子线程，并等待它们都完成一次倒计时后再继续执行。
 */
public class CountDownLatchDemo {
     ...
     * 主线程入口。
     *
     * await() 被中断时会抛出 InterruptedException，
     * 因此这里声明 throws InterruptedException。
     */
    public static void main(String[] args) throws InterruptedException {
        /*
         * 创建倒计时门闩，初始计数值为 3。
         *
         * 每调用一次 countDown()，计数减 1；
         * 当计数减到 0 时，所有正在 await() 等待的线程都会被唤醒。
         */
        ...
        new Thread(() -> {
            ...
            /*
             * 输出当前剩余计数。
             * 注意：多线程执行顺序不固定，因此输出顺序及 getCount() 的结果不应作为严格判断依据。
             */
            System.out.println(
                    Thread.currentThread().getName() + "-end..." + latch.getCount()
            );
        }).start();
         ...
         * 主线程阻塞等待。
         *
         * 只有当三个子线程都执行过 countDown()，
         * 即 latch 的计数从 3 减到 0 后，主线程才会继续向下执行。
         */
        latch.await();

        /*
         * 计数已归零，说明三个子线程都已经完成了倒计时。
         *
         * 注意：最后一个子线程调用 countDown() 后，
         * 主线程与该子线程谁先输出日志并不固定。
         */
...
<img src="images/image-20230505223219951.png" alt="image-20230505223219951" />

详细实现流程：

<img src="images/image-20230505223246059.png" alt="image-20230505223246059" />
...
<img src="images/image-20230505223442924.png" alt="image-20230505223442924" />
...
- 报表汇总
  ...
  <img src="images/image-20230505223536657.png" alt="image-20230505223536657" />



#### 4.1.4 案例二（异步调用）

<img src="images/image-20230505223640038.png" alt="image-20230505223640038" />
...
- `批量导入`：使用了线程池 + `CountDownLatch` 批量把数据库中的数据导入到了 ES（任意）中，避免 OOM。
- `数据汇总`：调用多个接口来汇总数据，如果所有接口（或部分接口）没有依赖关系，就可以使用线程池 + `Future` 来提升性能。
...
<img src="images/image-20230505224057228.png" alt="image-20230505224057228" />
...
<img src="images/image-20230505224341410.png" alt="image-20230505224341410" />
...
<img src="images/image-20230505224626253.png" alt="image-20230505224626253" />

**get方法/remove方法**

<img src="images/image-20230505224715087.png" alt="image-20230505224715087" />
...
<img src="images/image-20230505224755797.png" alt="image-20230505224755797" />

- `弱引用：`表示一个对象处于可能有用且非必须的状态。在GC线程扫描内存区域时，一旦发现弱引用，就会回收到弱引用相关联的对象。对于弱引用的回收，无关内存区域是否足够，一旦发现则会被回收

<img src="images/image-20230505224812015.png" alt="image-20230505224812015" />

每一个Thread维护一个ThreadLocalMap，在ThreadLocalMap中的Entry对象继承了WeakReference。其中key为使用弱引用的ThreadLocal实例，value为线程变量的副本

<img src="images/image-20230505224857538.png" alt="image-20230505224857538" />
...
`ThreadLocalMap` 中的 key 是弱引用，值为强引用；key 会被 GC 释放内存，关联 value 的内存并不会释放。建议主动 `remove` 释放 key、value。
...
>现在都是多核CPU，在多核CPU下
>
>`并发`是同一时间应对多件事情的能力，多个线程轮流使用一个或多个CPU
>
>`并行`是同一时间动手做多件事情的能力，4核CPU同时执行4个线程
>
>------
>
>**面试官**：说一下线程和进程的区别？
...
>> - 线程栈：局部变量、函数调用链
>> - 寄存器上下文：程序计数器、栈指针等
>> - 线程本地存储（TLS），例如 Java 的 `ThreadLocal`
>
>> 所以，一个线程在堆或全局变量中写的数据，其他线程通常能访问到，必须用锁、原子操作等保证并发安全；而函数里的普通局部变量通常在线程自己的栈上，其他线程不能直接共享。
>
>----
>
...
>
>还有一个就是，他们异常处理也不一样。Runnable接口run方法只能抛出运行时异常；Callable接口call方法允许抛出异常，可以获取异常信息
>
...
>**候选人：**
>
>在JDK中的Thread类中的枚举State里面定义了6中线程的状态分别是：新建、可运行、终结、阻塞、等待和计时等待六种。
>
...
>
>还有一种情况是调用 sleep(long) 方法也会从**可运行**状态进入**计时等待**状态，不需要主动唤醒，超时时间到自然恢复为**可运行**状态
>
...
>
>> 对，`ThreadPoolExecutor` 不是“四种线程池”之一，而是线程池的核心实现类。
>>
>> 这四个是 `Executors` 提供的工厂方法，内部大致关系如下：
>>
>> ```
>> newFixedThreadPool()      → ThreadPoolExecutor
>> newSingleThreadExecutor() → ThreadPoolExecutor（单线程配置，外层有包装）
...
>
>> 在ERP订单详情查询场景中，先获取订单主信息，再通过线程池并行查询客户、商品及发货信息，并利用Future获取各任务结果，最终汇总组装后返回完整订单详情，从而减少多个服务串行调用的等待时间。
>
>参考场景三：
>
>在我们商城里，有一个文章搜索的功能，用户输入关键字要搜索文章，同时需要保存用户的搜索记录（搜索历史），这块我设计的时候，为了不影响用户的正常搜索，我们采用的异步的方式进行保存的，为了提升性能，我们加入了线程池，也就说在调用异步方法的时候，直接从线程池中获取线程使用
