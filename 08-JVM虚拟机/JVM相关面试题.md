...
![image-20230506094254360](./images/image-20230506094254360.png)

**JVM由哪些部分组成，运行流程是什么？**

![image-20230506094411247](./images/image-20230506094411247.png)
...
![image-20230506094602329](./images/image-20230506094602329.png)

​	java虚拟机对于`多线程`是通过线程`轮流切换并且分配线程执行时间`。在任何的一个时间点上，一个处理器只会处理执行一个线程，如果当前被执行的这个线程它所分配的执行时间用完了`【挂起】`。处理器会切换到另外的一个线程上来进行执行。并且这个线程的执行时间用完了，接着处理器就会又来执行被挂起的这个线程。
...
![image-20230506094803545](./images/image-20230506094803545.png)

- `年轻代`被划分为三部分，Eden区和两个大小严格相同的Survivor区，根据JVM的策略，在经过几次垃圾收集后，任然存活于Survivor的对象将被移动到老年代区间。
- `老年`代主要保存生命周期长的对象，一般是一些老的对象
...
![image-20230506094938843](./images/image-20230506094938843.png)

##### 元空间(MetaSpace)介绍

​	在 HotSpot JVM 中，永久代（ ≈ 方法区）中用于存放类和方法的元数据以及常量池，比如Class 和 Method。每当一个类初次被加载的时候，它的元数据都会放到永久代中。

​	永久代是有大小限制的，因此如果加载的类太多，很有可能导致永久代`内存溢出`，即OutOfMemoryError，为此不得不对虚拟机做调优。
...
* `线程共享的区域：`主要用来保存`对象实例`、`数组`等，内存不够则抛出 OutOfMemoryError 异常。
* 组成：`年轻代 + 老年代`
  - `年轻代`被划分为三部分，Eden 区和两个大小严格相同的 Survivor 区
...
- 每个`线程运行时所需要的内存，称为虚拟机栈，先进后出`

- 每个栈由多个栈帧（frame）组成，对应着每次方法调用时所占用的内存

- 每个线程只能有一个活动栈帧，对应着当前正在执行的那个方法

![image-20230506095140595](./images/image-20230506095140595.png)

1. **垃圾回收是否涉及栈内存？**

   * `垃圾回收主要指就是堆内存`，当`栈帧弹栈以后，内存就会释放`
...
2. **栈内存分配越大越好吗？**
   ...
   * 栈帧过大会导致线程数变少，例如，机器总内存为512m，目前能活动的线程数则为512个，如果把栈内存改为2048k，那么能活动的栈帧就会减半
     ...
     * **解析：**每个线程都会分配一块栈空间，默认常见是约 `1024 KB`（实际值受 JVM、系统和配置影响）。栈越大，单个线程能承受更深的方法调用，较不容易出现 `StackOverflowError`。

       但总内存是有限的。假设可用于线程栈的内存总共 `512 MB`：

       ```
       每线程栈 1 MB  -> 最多约 512 个线程
       每线程栈 2 MB  -> 最多约 256 个线程
       ```

       所以栈调大一倍，能同时创建和运行的线程数大致会减半，过多时可能出现 `OutOfMemoryError: unable to create new native thread`。
...
3. **方法内的局部变量是否线程安全？**
   ...
   - 比如以下代码：
     ...
     ![image-20230506095306061](./images/image-20230506095306061.png)

> ⭐️**上述第三点解析**
>
> 
>
> 这页在说明：**局部变量本身在线程私有的栈帧里，但局部变量指向的对象可能在堆中被多个线程共享。**
>
> `m1()` 是线程安全的：
>
> ```
...
> ```
>
> 每次调用 `m1()` 都会新建一个独立的 `StringBuilder`，只在该方法内使用，方法结束后也没有把它交给外部。不同线程各用各的对象，不会互相干扰。
>
> `m2(sb)` 需要考虑线程安全：
>
> ```
...
> ```
>
> 这里的 `sb` 虽然是 `m2` 的局部参数，但它指向的是外部传进来的对象。若另一个线程也持有并同时修改同一个 `sb`，`StringBuilder` 就可能出现并发问题，因为它不是线程安全的。
>
> `m3()` 也需要考虑：
>
> ```
...
> ```
>
> 在 `m3()` 内部创建和修改 `sb` 时通常没问题；但它被 `return` 给外部后，对象“逃逸”出了方法。之后若多个线程共同使用返回值，就不安全了。
>
> 关键点：
>
> ```
> 局部变量的引用：在线程自己的栈中，天然隔离
> 局部变量指向的对象：通常在堆中，可能被多个线程共享
> ```
>
> 严格说，图中 `m2`、`m3` 不是“必然线程不安全”，而是对象可能被共享，所以必须进一步判断调用方式。

**栈内存溢出情况**

- 栈帧过多导致栈内存溢出，典型问题：递归调用

  ![image-20230506095401637](./images/image-20230506095401637.png)
...
> ⭐️**JVM 的“虚拟机栈”，可以把它理解成：每个线程执行方法时自己的调用记录本。**
>
> - 每个线程都有一块独立的虚拟机栈，遵循“后进先出”。
> - 每调用一个方法，JVM 就压入一个“栈帧（frame）”。
> - 栈帧会保存这个方法执行所需的信息，例如参数、局部变量、运算过程中的临时数据，以及方法结束后该回到哪里继续执行。
> - 方法调用结束，对应栈帧就弹出。
>
> 例如：
>
...
3. **栈内存分配越大越好吗？**
   * 未必，默认的栈内存通常为 1024k，栈帧过大会导致线程数变少。
...
6. *堆栈的区别是什么？*
   - `栈内存`一般会用来`存储局部变量和方法调用`，但`堆内存`是用来`存储 Java 对象和数组的`。`堆`会 GC `垃圾回收`，而`栈不会`。
...
![image-20230506095504213](./images/image-20230506095504213.png)
...
下图，左侧是`main方法的指令信息`，右侧`constant pool  是常量池`

main方法按照指令执行的时候，需要到常量池中查表翻译找到具体的类和方法地址去执行

![image-20230506095634842](./images/image-20230506095634842.png)

#### 1.5.3 运行时常量池

常量池是 `*.class` 文件中的，当该类被加载，它的常量池信息就会`放入运行时常量池`，并把里面的`符号地址变为真实地址`

![image-20230506100142724](./images/image-20230506100142724.png)
...
![image-20230506100501905](./images/image-20230506100501905.png)
...
![image-20230506100548455](./images/image-20230506100548455.png)

下图是NIO传输数据的流程，在这个里面主要使用到了一个直接内存，不需要在堆中开辟空间进行数据的拷贝，jvm可以直接操作直接内存，从而使数据读写传输更快。

![image-20230506100621146](./images/image-20230506100621146.png)
...
![](./images/image-20220903233627146.png)
...
![image-20230506100746624](./images/image-20230506100746624.png)
...
![image-20230506100920042](./images/image-20230506100920042.png)
...
![image-20220903144547378](./images/image-20220903144547378.png)
...
![image-20230506101032605](./images/image-20230506101032605.png)

**类加载过程详解**

1.加载

![image-20230506101115674](./images/image-20230506101115674.png) 
...
![image-20230506101213373](./images/image-20230506101213373.png)

2.验证

![image-20230506101420202](./images/image-20230506101420202.png)
...
![image-20230506101445898](./images/image-20230506101445898.png)
...
![image-20230506101824622](./images/image-20230506101824622.png)

4.解析

![image-20230506101504632](./images/image-20230506101504632.png)

**把类中的符号引用转换为直接引用**

比如：方法中调用了其他方法，方法名可以理解为符号引用，而直接引用就是使用指针直接指向方法。

![image-20230506102311951](./images/image-20230506102311951.png)

5.初始化

![image-20230506101625087](./images/image-20230506101625087.png)
...
![image-20230506101641837](./images/image-20230506101641837.png)
...
![image-20230506104954777](./images/image-20230506104954777.png)
...
![image-20230506111102825](./images/image-20230506111102825.png)

```java
String demo = null;
```

![image-20230506111136231](./images/image-20230506111136231.png)

当对象间出现了循环引用的话，则引用计数法就会失效

![image-20230506111255401](./images/image-20230506111255401.png)

先执行右侧代码的前4行代码

![image-20230506111327590](./images/image-20230506111327590.png)

目前上方的引用关系和计数都是没问题的，但是，如果代码继续往下执行，如下图

![image-20230506111512450](./images/image-20230506111512450.png)
...
![image-20220904010634153](./images/image-20220904010634153.png)
...
![image-20230506112047190](./images/image-20230506112047190.png)
...
![image-20230506111919008](./images/image-20230506111919008.png)
...
![image-20230506111957793](./images/image-20230506111957793.png)
...
![image-20230506131229649](./images/image-20230506131229649.png)
...
![image-20230506131308654](./images/image-20230506131308654.png)

- 新创建的对象，都会先分配到eden区

![image-20230506131415418](./images/image-20230506131415418.png)

- 当伊甸园内存不足，标记伊甸园与 from（现阶段没有）的存活对象

- 将存活对象采用复制算法复制到 to 中，复制完毕后，伊甸园和 from 内存都得到释放

![image-20230506131442503](./images/image-20230506131442503.png)

- 经过一段时间后伊甸园的内存又出现不足，标记eden区域to区存活的对象，将存活的对象复制到from区

![image-20230506131544447](./images/image-20230506131544447.png)

![image-20230506131607645](./images/image-20230506131607645.png)

- 当幸存区对象熬过几次回收（最多15次），晋升到老年代（幸存区内存不足或大对象会导致提前晋升）

**MinorGC、 Mixed GC 、 FullGC的区别是什么**

![image-20230506131640893](./images/image-20230506131640893.png)
...
![image-20230506154006266](./images/image-20230506154006266.png)
...
![image-20230506154042673](./images/image-20230506154042673.png)

#### 3.5.2 CMS（并发）垃圾收集器

CMS全称 Concurrent Mark Sweep，是一款并发的、使用标记-清除算法的垃圾回收器，该回收器是针对老年代垃圾回收的，是一款以获取最短回收停顿时间为目标的收集器，停顿时间短，用户体验就好。其最大特点是在进行垃圾回收时，应用仍然能正常运行。

![image-20230506154117857](./images/image-20230506154117857.png)

![image-20230506154107944](./images/image-20230506154107944.png)
...
![image-20230506154323950](./images/image-20230506154323950.png)

#### 3.6.2 Young Collection(年轻代垃圾回收)

- 初始时，所有区域都处于空闲状态

  ![image-20230506154542687](./images/image-20230506154542687.png)

- 创建了一些对象，挑出一些空闲区域作为伊甸园区存储这些对象

  ![image-20230506154607558](./images/image-20230506154607558.png)

- 当伊甸园需要垃圾回收时，挑出一个空闲区域作为幸存区，用复制算法复制存活对象，需要暂停用户线程

  ![image-20230506154633118](./images/image-20230506154633118.png)

  ![image-20230506154705088](./images/image-20230506154705088.png)

- 随着时间流逝，伊甸园的内存又有不足

- 将伊甸园以及之前幸存区中的存活对象，采用复制算法，复制到新的幸存区，其中较老对象晋升至老年代

  ![image-20230506154759809](./images/image-20230506154759809.png)

  ![image-20230506154826981](./images/image-20230506154826981.png)

  ![image-20230506154859985](./images/image-20230506154859985.png)
...
![image-20230506155000503](./images/image-20230506155000503.png)

- 并发标记之后，会有重新标记阶段解决漏标问题，此时需要暂停用户线程。

- 这些都完成后就知道了老年代有哪些存活对象，随后进入混合收集阶段。此时不会对所有老年代区域进行回收，而是根据暂停时间目标优先回收价值高（存活对象少）的区域（这也是 Gabage First 名称的由来）。

  ![image-20230506155047765](./images/image-20230506155047765.png)
...
![image-20230506155116267](./images/image-20230506155116267.png)

其中H叫做巨型对象，如果对象非常大，会开辟一块连续的空间存储巨型对象

![image-20230506155146370](./images/image-20230506155146370.png)
...
![image-20230506155341703](./images/image-20230506155341703.png)
...
![image-20230506155416293](./images/image-20230506155416293.png)
...
![image-20230506155501557](./images/image-20230506155501557.png)
...
![image-20230506155518510](./images/image-20230506155518510.png)

![image-20230506155552693](./images/image-20230506155552693.png)
...
![image-20220904151948778](./images/image-20220904151948778.png)
...
![image-20220904104739581](./images/image-20220904104739581.png)
...
![image-20220904111059602](./images/image-20220904111059602.png)
...
![image-20220904114511854](./images/image-20220904114511854.png)
...
![image-20220904115157363](./images/image-20220904115157363.png)
...
![image-20220904115936095](./images/image-20220904115936095.png)

可以内存、线程、类等信息

![image-20220904120057211](./images/image-20220904120057211.png)
...
![image-20220904120356174](./images/image-20220904120356174.png)

监控程序运行情况

![image-20220904132011289](./images/image-20220904132011289.png)

查看运行中的dump

![image-20220904132134095](./images/image-20220904132134095.png)

查看堆中的信息

![image-20220904132346495](./images/image-20220904132346495.png)
...
![image-20230506155704119](./images/image-20230506155704119.png)
...
![image-20220904132925812](./images/image-20220904132925812.png)

3、通过查看堆信息的情况，可以大概定位内存溢出是哪行代码出了问题

![image-20220904133722905](./images/image-20220904133722905.png)
...
![image-20220904161818255](./images/image-20220904161818255.png)
...
![image-20220904162117022](./images/image-20220904162117022.png)
...
>`printf "%x\n" 30979`
>
>![image-20220904162654928](./images/image-20220904162654928.png)
...
![image-20220904162941977](./images/image-20220904162941977.png)
